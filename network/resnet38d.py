...
class CausalCounterfactualAttention(nn.Module):
    """
    C²AM: Causal Counterfactual Reasoning Module
    Paper-ready, plug-and-play
    """

    def __init__(self, channels, num_factors=8, reduction=4):
        super().__init__()
        self.C = channels
        self.K = num_factors

        assert channels % num_factors == 0, \
            "channels must be divisible by num_factors"

        self.factor_dim = channels // num_factors

        # 1️⃣ 因果因素投影
        self.factor_proj = nn.Conv2d(
            channels,
            channels,
            kernel_size=1,
            bias=False
        ...
        self.causal_matrix = nn.Parameter(
            torch.zeros(num_factors, num_factors)
        )
        nn.init.xavier_uniform_(self.causal_matrix)
    ...
    def forward(self, x):
        ...
        # === Step 1: 因果因素分解 ===
        z = self.factor_proj(x)
        z = z.view(B, self.K, self.factor_dim, H, W)

        # 全局因果统计（弱监督友好）
        z_pool = z.mean(dim=[3, 4])  # [B, K, D]

        # === Step 2: 因果反事实干预 ===
        # A: [K, K]
        A = F.softmax(self.causal_matrix, dim=-1)

        # 计算因果影响
        causal_effect = torch.einsum(
            'ij,bjd->bid',
            A,
            z_pool
        )
        ...
        # 反事实抑制
        z_cf = z_pool - causal_effect
        z_cf = z_cf.unsqueeze(-1).unsqueeze(-1)

        # broadcast 回空间
        z_cf = z_cf.expand_as(z)

        # === Step 3: 重构与残差 ===
        z_cf = z_cf.contiguous().view(B, C, H, W)
        out = self.reconstruct(z_cf)
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.c2am1 = CausalCounterfactualAttention(512,8)
        self.c2am2 = CausalCounterfactualAttention(1024,8)
        self.c2am = CausalCounterfactualAttention(2048, 8)
        self.esf = ISEF(2048)
