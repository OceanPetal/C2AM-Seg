...
from tool.UGML import AdaptiveMarginConfidenceLoss
from network.CNN_DINO import  DualPath_CNN_DINO_Segmentor
...
class Trainer(object):
    def __init__(self, args):
        ...
        model = DualPath_CNN_DINO_Segmentor(
            nclass=self.nclass,
            dino_weight_path=r"H:\class4\DINOv3\DINOV3-pth\dinov3_vitb16_pretrain_lvd1689m-73cec8be.pth"
        ).cuda()
        ...
        for name, p in model.named_parameters():
            if not p.requires_grad:
                continue
            if 'vit_branch.adapter' in name or 'fusion' in name or 'head' in name:
        ...
        self.optimizer_sem = torch.optim.AdamW(
        semantic_params,
        lr=lr * 0.5,
        weight_decay=1e-4
)
        self.criterion = AdaptiveMarginConfidenceLoss(
    ...
    def training(self, epoch):
        ...
        tbar = tqdm(self.train_loader, total=len(self.train_loader), colour='magenta')
    ...
    def validation(self, epoch):
        ...
        tbar = tqdm(self.val_loader, colour='magenta')
        all_biou = []
        for i, sample in enumerate(tbar):
            ...
            for b in range(pred.shape[0]):
                biou, _ = self.evaluator.Boundary_IoU(target[b], pred[b])
        ...
        mean_biou = np.mean(all_biou)
        self.writer.add_scalar('val/mIoU', mIoU, epoch)
        self.writer.add_scalar('val/Acc', Acc, epoch)
        self.writer.add_scalar('val/Acc_class', Acc_class, epoch)
        self.writer.add_scalar('val/fwIoU', FWIoU, epoch)
        self.writer.add_scalar('val/Dice', MDice, epoch)
        self.writer.add_scalar('val/Boundary_IoU', mean_biou, epoch)
        print('Validation:')
        print('[Epoch: %d, numImages: %5d]' % (epoch, i * self.args.batch_size + image.data.shape[0]))
        print("Acc: {}, Acc_class: {}, mIoU: {}, fwIoU: {}, Mean Dice: {}, Mean Boundary IoU: {}".format(
        Acc, Acc_class, mIoU, FWIoU, MDice, mean_biou))
    ...
    def test(self, epoch, args):
        ...
        self.model.eval()
        self.evaluator.reset()
        tbar = tqdm(self.test_loader, colour='magenta')
        all_biou = []
        for i, sample in enumerate(tbar):
            image, target = sample[0]['image'], sample[0]['label']
            ...
            # save_path = os.path.join('./datasets/BCSS-WSSS/test/pred/', png_name + '.png')
            # save_dir = os.path.dirname(save_path)
            # os.makedirs(save_dir, exist_ok=True)
            # visualimg.save(save_path, format='PNG')
            self.evaluator.add_batch(target, pred)
            for b in range(pred.shape[0]):
                # biou, _ = self.evaluator.Boundary_IoU(target[b], pred[b], ignore_index=4)
                biou, _ = self.evaluator.Boundary_IoU(target[b], pred[b])
                all_biou.append(biou)
